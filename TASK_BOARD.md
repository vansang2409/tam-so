...
| DC12 | Hậu cảnh modal Xin Xăm dùng ẢNH THẬT (đã có sẵn 11 ảnh) thay nền phẳng, theo góp ý "chỉnh background như hình trước đó" | UI | Med | Done (v3.84) | `DcModal` thêm prop `bgImage`; ảnh đúng khu đang xem, blur+tối để chữ đọc được; verify ảnh tải thành công + computed style qua dev server thật |
