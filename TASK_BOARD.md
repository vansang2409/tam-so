...
| NT03 | Bo popup chinh giua Di Nha Tho | UI | High | Done (v4.3.10) | Card chinh giua scene khong con nen/border/shadow; noi dung noi tren anh khu, giu CTA va nen |
| NT02 | Moi khu Di Nha Tho co anh rieng va chuyen canh 3D | Feature/UI | High | Done (v4.3.9) | Them 5 anh AI, toi uu webp/jpg/png, `NHATHO_SPACES.image`, background + khung anh 3D doi theo khu |
| NT01 | Them menu Di Nha Tho voi anh tu tao va khong gian chiem niem online | Feature/UI | High | Done (v4.3.8) | `/di-nha-tho` full-page, 6 khu, thap nen, ghi chu localStorage, asset `public/nhatho/nha-tho-tam-so.png`, wire Home/Layout/routeLoaders/sitemap; 473 test pass |
...
| H3D01 | Nang cap menu Trang chu thanh vong chon 3D | UI/3D | High | Done (v4.3.0) | `Home3DMenu.jsx` dung Three.js/WebGL ve vong la ban/portal, 6 link DOM that, lazy-load sau hero, fallback 2D/reduced-motion; `three` van tach chunk rieng; 446 test pass; build `/tmp/b` main+single OK; Playwright 1366/390/320 canvas nonblank, 0 overlap, 0 overflow ngang |
| H3D02 | Sua overflow header mobile lo ra khi verify Home 3D | UX/Nav | High | Done (v4.3.0) | AuthBtn compact duoi `sm`, nut bo suu tap/menu giam padding, logo an chu o <=340px; Playwright mobile/small-mobile `overflowX=false` |
## PHIEN 30/06/2026 - DI CHUA 3D
| ID | Task | Nhom | Uu tien | TT | Tieu chi xong |
|---|---|---|---|---|---|
...
| P5 | PWA cache cho chunk dong | Perf/PWA | High | Done (v4.1.6) | service worker tach navigation network-first, `/assets/*.js/css/map` stale-while-revalidate, media/font cache-first; fallback offline giu route da cache; 426 test pass; build OK |
...
| AUTH04 | Sync Ho so tong hop bang Supabase DB/RLS | Account/Supabase | High | Done (v4.1.4) | `supabase/schema.sql` tao `public.profiles` lien ket `auth.users(id)` + RLS owner-only; `/ho-so` tai/luu profile khi da dang nhap, fallback localStorage cho khach; an banner sync ky thuat o trang thai binh thuong, chi canh bao khi loi; 421 test pass; build `/tmp/b` main+single OK, single NUL=0 |
| NAV01 | Bo sung menu Trang chu tren desktop nav | UX/Nav | High | Done (v4.1.5) | `Trang chu` hien ro trong nav desktop, van giu logo link; mobile da co san; 422 test pass; build `/tmp/b` main+single OK, single NUL=0 |
