...
| AUTH04 | Sync Ho so tong hop bang Supabase DB/RLS | Account/Supabase | High | Done (v4.1.3) | `supabase/schema.sql` tao `public.profiles` lien ket `auth.users(id)` + RLS owner-only; `/ho-so` tai/luu profile khi da dang nhap, fallback localStorage cho khach; 420 test pass; build `/tmp/b` main+single OK, single NUL=0 |
