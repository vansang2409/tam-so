...
// — Đi chùa: dữ liệu khu + bộ thẻ xăm (thuần JS, test bằng Node) —
{
  ...
  const home3dHome = readFileSync(new URL('../src/pages/Home.jsx', import.meta.url), 'utf8')
  ...
  ok(tarot3d.includes('tarot-library-grid') && tarot3d.includes('tarot-library-card') && tarot3d.includes('tarotAccent(c)') && tarot3d.includes('tarot-library-image') && tarot3d.includes('tarot-fav-btn'), 'Tarot.jsx: thu vien 78 la co card deck 3D rieng, anh/meta/fav noi lop')
  ok(route3dCss.includes('.tarot-library-card:hover') && route3dCss.includes('translate3d(var(--spread), -10px, 46px)') && route3dCss.includes('@keyframes tarot-card-rise') && route3dCss.includes('tarot-card-rise .64s cubic-bezier(.18,.8,.25,1) backwards') && route3dCss.includes('.tarot-library-grid.in > .tarot-library-card') && route3dCss.includes('.tarot-library-aura') && route3dCss.includes('.tarot-filter-pill.is-active'), 'CSS: Tarot library co hover 3D manh, aura, filter pill va animation rise')
  ...
  ok(layCS.includes('prefetchRouteChunk') && layCS.includes('onMouseEnter') && layCS.includes('onFocus') && layCS.includes('prefetchProps'), 'Layout.jsx: nav prefetch chunk khi hover/focus')
  const eb = readFileSync(new URL('../src/components/ErrorBoundary.jsx', import.meta.url), 'utf8')
  ok(eb.includes('isChunkLoadError') && eb.includes('Failed to fetch dynamically imported module') && eb.includes('ChunkLoadError'), 'ErrorBoundary: nhan dien lazy chunk fail')
...
// Supabase Auth: đăng ký/đăng nhập tuỳ chọn
{
  ...
  ok(pkgAuth.version === '4.3.5', 'package.json: bump version v4.3.5 cho Tarot library 3D deck')
  ...
  ok(authPage.includes('supabase.auth.signInWithOAuth') && authPage.includes("provider: 'google'") && authPage.includes("redirectTo: authRedirectTo('/dang-nhap')"), 'Auth.jsx: co Google OAuth qua Supabase signInWithOAuth')
  ok(authPage.includes('auth-google') && authPage.includes('Tiếp tục với Google') && authPage.includes('hoặc dùng email') && authPage.includes('Google chưa được bật'), 'Auth.jsx: UI Google login + divider + loi cau hinh provider')
...
// PWA runtime cache: route chunks sau code-split phai cache duoc khi offline/doi deploy
{
  ...
  ok(sw.includes("const CACHE = 'tamso-v4.3.5'"), 'sw.js: bump CACHE v4.3.5')
