...
export function hopTuoiChi(c1, c2) {
  const inG = groups => groups.some(g => g.includes(c1) && g.includes(c2))
  if (c1 === c2) return 'Cùng tuổi'
  if (inG(TAM_HOP)) return 'Tam hợp'
  if (inG(LUC_HOP)) return 'Lục hợp'
  if (inG(LUC_XUNG)) return 'Lục xung'
  if (inG(TU_HANH_XUNG)) return 'Tứ hành xung'
  return 'Bình thường'
}
...
/* ===== Sao hạn Cửu Diệu (sao chiếu mệnh theo tuổi mụ + giới tính) =====
 * Chu kỳ 9 năm, bắt đầu tuổi 10. Thứ tự nam/nữ suy ra & ĐỐI CHIẾU CHÉO từ
 * bảng sao hạn 2026 (quantrimang) — nhiều mốc khớp. Quan niệm dân gian, tham khảo. */
export const SAO_HAN = {
  'La Hầu': { loai: 'xấu', y: 'Hung tinh cho cả nam & nữ — dễ thị phi, hao của, sức khỏe sa sút; lưu ý tháng Giêng & tháng 2 âm.' },
  'Thổ Tú': { loai: 'trung', y: 'Sao trung — coi chừng tiểu nhân gièm pha, gia đạo, xuất hành; xấu nhất tháng 4 & 8 âm.' },
  'Thủy Diệu': { loai: 'trung', y: 'Phúc lộc tinh (cát nhiều hơn hung) — tài lộc, hỉ sự; giữ lời nói & kiêng sông nước, nhất là nữ.' },
  'Thái Bạch': { loai: 'xấu', y: 'Sao xấu nhất, bất lợi cho nam — hao tài "sạch cửa nhà"; lưu ý tháng 2, 5, 8 âm.' },
  'Thái Dương': { loai: 'tốt', y: 'Cát tinh, rất tốt cho nam (công danh, may mắn); nữ giữ sức khỏe tháng 6 & 10.' },
  'Vân Hớn': { loai: 'trung', y: 'Sao trung — dễ cãi vã, thị phi; nam coi chừng pháp luật, nữ lưu ý thai sản; xấu tháng 2 & 8.' },
  'Kế Đô': { loai: 'xấu', y: 'Hung tinh của nữ — buồn phiền, hao tài, tình cảm trắc trở; xấu nhất tháng 3 & 9 âm.' },
  'Thái Âm': { loai: 'tốt', y: 'Cát tinh cho cả nam & nữ — tài lộc, quý nhân phù trợ, hỉ sự (tháng 9).' },
  'Mộc Đức': { loai: 'tốt', y: 'Phúc tinh — an lành, may mắn, hỉ sự; đặc biệt tốt tháng 10 & tháng Chạp.' }
}
const _SAO_NAM = ['La Hầu', 'Thổ Tú', 'Thủy Diệu', 'Thái Bạch', 'Thái Dương', 'Vân Hớn', 'Kế Đô', 'Thái Âm', 'Mộc Đức']
const _SAO_NU = ['Kế Đô', 'Vân Hớn', 'Mộc Đức', 'Thái Âm', 'Thổ Tú', 'La Hầu', 'Thái Dương', 'Thái Bạch', 'Thủy Diệu']
/** Sao chiếu mệnh theo TUỔI MỤ (âm) + giới tính ('nam'|'nu'). Chu kỳ từ 10 tuổi. */
export function saoHan(tuoiAm, gioiTinh) {
  if (!tuoiAm || tuoiAm < 1) return null
  const idx = (((tuoiAm - 10) % 9) + 9) % 9
